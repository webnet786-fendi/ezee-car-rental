...
function auth_(p) { return !!(p && p.token && ADMIN_TOKEN && ADMIN_TOKEN !== 'REPLACE_WITH_YOUR_SECRET' && p.token === ADMIN_TOKEN); }
...
function doGet(e) {
  var p = (e && e.parameter) || {};
  if (p.action === 'ping') return out_({ ok: true, service: 'ezee-bookings', time: new Date().toISOString() });
  if (p.action === 'list') {
